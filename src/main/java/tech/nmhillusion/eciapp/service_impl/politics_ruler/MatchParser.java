package tech.nmhillusion.eciapp.service_impl.politics_ruler;

import tech.nmhillusion.eciapp.model.politics_ruler.PoliticianEntity;
...
class MatchParser {
    ...
    private Optional<PoliticianEntity> parseCharacterParagraph(String paragraph) {
        ...
        if (firstParsed.isPresent()) {
            final String originalParagraph = HtmlUtil.htmlUnescape(
                    StringUtil.trimWithNull(
                            firstParsed.get().get(0)
                    )
            );

            final String fullName = HtmlUtil.htmlUnescape(
                    StringUtil.trimWithNull(
                            firstParsed.get().get(1)
                    )
            );
            final String lifeTime = HtmlUtil.htmlUnescape(
                    StringUtil.trimWithNull(
                            firstParsed.get().get(2)
                    )
            );
            final String role = HtmlUtil.htmlUnescape(
                    StringUtil.trimWithNull(
                            firstParsed.get().get(3)
                    )
            );
            final String note = HtmlUtil.htmlUnescape(
                    StringUtil.trimWithNull(
